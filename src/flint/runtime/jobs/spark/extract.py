...
class ExtractSpark(ExtractModel, ABC):
    ...
    def extract(self) -> None:
        ...
        """
        logger.info("Starting extraction for source: %s using method: %s", self.name, self.method.value)
        ...
        if self.method == ExtractMethod.BATCH:
            logger.debug("Performing batch extraction for: %s", self.name)
            self.data_registry[self.name] = self._extract_batch()
            logger.info("Batch extraction completed successfully for: %s", self.name)
        elif self.method == ExtractMethod.STREAMING:
            logger.debug("Performing streaming extraction for: %s", self.name)
            self.data_registry[self.name] = self._extract_streaming()
            logger.info("Streaming extraction completed successfully for: %s", self.name)
        else:
