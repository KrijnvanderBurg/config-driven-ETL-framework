...
from samara.runtime.jobs.models.transforms.model_groupby import GroupByArgs
...
class TestGroupByFunctionValidation:
    ...
    def test_create_groupby_function__non_count_with_null_input_column__succeeds_model_validation(self) -> None:
        """Test GroupByFunction creation allows null input_column for non-count (validated at runtime)."""
        config = {
            "function_type": "groupby",
            "arguments": {
                "group_columns": ["category"],
        ...
        # Act - should create successfully (validation happens at runtime)
        groupby_function = GroupByFunction(**config)
